...
async function imageResize(size, image, imageout, extType) {
	try {
		// Для AVIF файлов используем специальную обработку
		if (/avif/i.test(extType)) {
			// Если исходный файл уже AVIF, просто копируем его без обработки
			if (fs.existsSync(image)) {
				await fs.promises.copyFile(image, imageout);
				if (!size) {
		...
		let pipeline = sharp(image, { animated: true, failOnError: false }).resize(size);
		...
		} else if (/avif/i.test(extType)) {
			// Если исходный файл AVIF и не нужно конвертировать, просто копируем
			if (fs.existsSync(image)) {
				await fs.promises.copyFile(image, imageout);
				if (!size) {
					copyIgnore.add(imageout.replace('dist', ''));
		...
		// Если ошибка при обработке AVIF, просто копируем файл
		if (/avif|heif/i.test(extType) && fs.existsSync(image)) {
			console.warn(`Warning: Could not process ${extType} image ${image}, copying as-is:`, error.message);
			await fs.promises.copyFile(image, imageout);
			if (!size) {
				copyIgnore.add(imageout.replace('dist', ''));
			}
